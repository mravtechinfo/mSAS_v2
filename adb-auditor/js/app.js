...
(function() {
    ...
    const templates = {
                ...
                <footer class="landing-footer">
                    <div class="landing-footer-inner">
                        <div class="footer-brand">
                            ...
                            <a href="https://github.com/thecybersandeep/adbauditor" target="_blank" rel="noopener noreferrer">GitHub</a>
                        </div>
                        <div class="footer-copyright">by <a href="https://www.linkedin.com/in/sandeepwawdane/" target="_blank" rel="noopener noreferrer">Sandeep</a></div>
                    </div>
                </footer>
            </div>`,
        overview: `
        ...
        security: `
            <div class="page">
                ...
                <div class="tab-bar">
                    <button class="tab-btn active" data-tab="fullScan" onclick="app.switchSecurityTab('fullScan')">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                        Full Scan
                    </button>
                    <button class="tab-btn" data-tab="localStorage" onclick="app.switchSecurityTab('localStorage')">
        ...
        fullScan: `
            <div class="security-panel">
                <div class="panel-header">
                    ...
                    <span class="badge warning">All checks</span>
                </div>
    ...
    function exportFindings(kind) {
        ...
        if (kind === 'json') {
            ...
            const findings = data.findings || data.tests || [];
            const rules = new Map();
            const results = [];
            for (const f of findings) {
                const id = f.id || f.type || ('rule_' + rules.size);
                if (!rules.has(id)) rules.set(id, {
                    id, name: f.name || id,
                    shortDescription: { text: f.name || id },
                    fullDescription: { text: f.description || f.name || id },
                    defaultConfiguration: { level: levelFor(f.severity) },
                });
                results.push({
                    ruleId: id,
                    level: levelFor(f.severity),
                    message: { text: f.description || f.name || id },
                    locations: f.location ? [{ physicalLocation: { artifactLocation: { uri: String(f.location) } } }] : [],
                    properties: { severity: f.severity, package: data.package, finding: f },
                });
            }
            const sarif = {
                ...
                version: '2.1.0',
                runs: [{
                    tool: { driver: { name: 'ADB Auditor', version: '2.0', informationUri: '', rules: [...rules.values()] } },
                    results,
                    properties: { package: data.package, summary: data.summary },
                }],
            };
            downloadBlob(JSON.stringify(sarif, null, 2), pkg + '_' + ts + '.sarif', 'application/json');
        }
    }
    ...
    function showExportMenu(anchor) {
        ...
        menu.innerHTML = '<button data-fmt="pdf"   type="button"><span>PDF Report</span><small>printable audit, every instance</small></button>' +
                         '<button data-fmt="json"  type="button"><span>JSON</span><small>full result tree</small></button>' +
                         '<button data-fmt="sarif" type="button"><span>SARIF 2.1</span><small>GitHub Code Scanning</small></button>';
    ...
    function prettyAppName(pkg) {
        ...
        const generic = new Set(['android', 'app', 'apps', 'mobile', 'client', 'main', 'release', 'debug', 'dev', 'pro', 'free', 'lite', 'demo', 'beta', 'core']);
        for (let i = parts.length - 1; i >= 0; i--) {
            const seg = parts[i];
            if (seg && !generic.has(seg.toLowerCase())) {
    ...
    let landingHeroMoveListener = null;
    function setupLandingInteractivity() {
        ...
        if (heroContent) {
            teardownLandingInteractivity();
            landingHeroMoveListener = (e) => {
            ...
            document.addEventListener('mousemove', landingHeroMoveListener);
        }
    }
    function teardownLandingInteractivity() {
        if (landingHeroMoveListener) {
            document.removeEventListener('mousemove', landingHeroMoveListener);
            landingHeroMoveListener = null;
        }
    }
    ...
    async function setupSecurity() {
        app.switchSecurityTab('fullScan');
    ...
    const app = {
        ...
        async connect() {
            try {
                ...
                const helpKey = e && e.helpKey;
                const lower = msg.toLowerCase();
                if (helpKey === 'claim' || lower.includes('claim') || lower.includes('busy') || lower.includes('unable to claim') || lower.includes('access denied')) {
                    showClaimErrorModal(e.original || msg);
                } else {
        ...
        async disconnect() {
            this.stopLogcat();
            if (adb) {
                try { await adb.disconnect(); } catch (e) { console.warn('Disconnect cleanup failed:', e && e.message); }
                adb = null;
            }
            state.connected = false;
            state.shellCwd = '';
        ...
        async loadOverview() {
            ...
            try {
                ...
                } catch (e) { console.warn('Storage probe failed:', e && e.message); }
                statsGrid.innerHTML = `
                    <div class="stat-card">
                        <div class="stat-label">Android</div>
                        <div class="stat-value">${escapeHtml(props.androidVersion || '-')}</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-label">SDK Level</div>
                        <div class="stat-value">${escapeHtml(props.sdkVersion || '-')}</div>
                    </div>
                    <div class="stat-card">
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-value">${escapeHtml(storagePercent)}%</div>
                    </div>
                `;
                ...
                } catch (e) { console.warn('getprop probe failed:', e && e.message); }
                propList.innerHTML = [
                    ...
                    <div class="prop-item">
                        <span class="prop-label">${escapeHtml(p.label)}</span>
                        <span class="prop-value">${escapeHtml(p.value)}</span>
                    </div>
                `).join('');
            } catch (e) {
            ...
            list.innerHTML = apps.map(a => `
                <div class="app-item">
                    ...
                    <div class="app-actions">
                        <button class="btn btn-sm btn-ghost apk-download-btn" data-pkg="${escapeAttr(a.package)}" title="Download APK">
                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                        </button>
                    </div>
                </div>
            ...
            if (pathBar) {
                ...
                pathBar.innerHTML = `
                    ...
                    ${segments.map((seg, i) => {
                        ...
                        return `<span class="path-sep">/</span><span class="path-segment" data-path="${escapeAttr(segPath)}">${escapeHtml(seg)}</span>`;
                    }).join('')}
                `;
        ...
        switchSecurityTab(tab) {
            ...
            // Save the current selection before switching tabs
            const prevSelectId = selectMap[state.currentSecurityTab];
            const prevSelect = document.getElementById(prevSelectId);
            if (prevSelect && prevSelect.value) {
                state.selectedPackage = prevSelect.value;
            }
                ...
                if (selectId) {
                    ...
                    if (select) {
                        if (state.securityApps && state.securityApps.length > 0) {
                            ...
                            // Restore the previously selected package
                            if (state.selectedPackage) {
                                select.value = state.selectedPackage;
                            }
                        } else {
                        ...
                        for (const f of data[section.key]) {
                            ...
                            html += `<div class="file-row">
                                ...
                                <div class="file-row-actions">
                                    <button class="btn btn-sm btn-ghost storage-view-btn" data-path="${encodedPath}">View</button>
                                    <button class="btn btn-sm btn-ghost storage-dl-btn" data-pkg="${encodedPkg}" data-path="${encodedPath}" data-name="${encodedName}">Download</button>
                                </div>
                            </div>`;
                        }
                        html += '</div></div>';
        ...
        async checkBackup() {
            ...
            try {
                ...
                results.innerHTML = `<div class="result-box"><div class="result-header"><span class="badge ${badge}">${label}</span></div><p style="margin-top:12px;color:var(--lum-70)">${escapeHtml(result.description || '')}</p></div>`;
            } catch (e) {
                results.innerHTML = `<div class="result-box"><span class="badge fail">Error</span><p style="margin-top:8px">${escapeHtml(e.message)}</p></div>`;
            }
        },
                    ...
                    for (const f of result.findings) {
                        if (f.components) {
                            ...
                            for (const c of f.components) {
                                hasComponents = true;
                                html += `<div class="component-item"><span class="component-type ${escapeAttr(type)}">${escapeHtml(type)}</span><span class="component-name">${escapeHtml(c)}</span></div>`;
                            }
                        }
                    }
                }
        ...
        async runFullScan() {
            ...
            try {
                ...
                html += `<div class="result-header"><strong>Scan Report: ${escapeHtml(pkg)}</strong><span class="result-timestamp">${escapeHtml(scanResult.timestamp)}</span></div>`;
                html += '<div class="scan-summary-grid">';
                html += `<div class="scan-summary-card pass"><div class="scan-summary-value">${scanResult.summary.pass}</div><div class="scan-summary-label">Pass</div></div>`;
                html += `<div class="scan-summary-card fail"><div class="scan-summary-value">${scanResult.summary.fail}</div><div class="scan-summary-label">Fail</div></div>`;
                html += `<div class="scan-summary-card review"><div class="scan-summary-value">${scanResult.summary.review}</div><div class="scan-summary-label">Review</div></div>`;
                html += `<div class="scan-summary-card error"><div class="scan-summary-value">${scanResult.summary.error}</div><div class="scan-summary-label">Error</div></div>`;
                html += '</div></div>';

                const order = { fail: 0, review: 1, pass: 2, error: 3 };
                const sortedTests = scanResult.tests.slice().sort((a, b) => (order[a.status] || 9) - (order[b.status] || 9));
                ...
                for (const test of sortedTests) {
                    ...
                    html += `<div class="result-box test-card test-${escapeAttr(test.status)}"><div class="result-header"><span class="badge ${badge}">${label}</span><strong class="test-title">${escapeHtml(test.title || test.id)}</strong>`;
                    if (test.id || test.category) {
                        const meta = [];
                        if (test.id) meta.push(test.id);
                        if (test.category) meta.push(test.category);
                        html += `<span class="test-meta">${escapeHtml(meta.join(' · '))}</span>`;
                    }
                    html += '</div>';
                    if (test.description) {
                        html += `<p class="test-desc">${escapeHtml(test.description)}</p>`;
                    }
                    const findings = (test.findings || []).filter(f => {
                        const key = (f.type || '') + '|' + (f.file || '') + '|' + (f.note || '') + '|' + (f.location || '');
                        if (seenFindings.has(test.id + '||' + key)) return false;
                        seenFindings.add(test.id + '||' + key);
                        return true;
                    });
                    if (findings.length > 0) {
                        ...
                        for (let i = 0; i < findings.length; i++) {
                            ...
                            const sevClass = sev === 'critical' || sev === 'high' ? 'high' : sev === 'medium' ? 'med' : sev === 'low' ? 'low' : 'info';
                            html += `<article class="finding-row ${sevClass}">`;
                            html += `<div class="finding-row-head"><span class="finding-num">#${i + 1}</span><span class="finding-type">${escapeHtml((f.type || 'finding').replace(/_/g, ' '))}</span>`;
                            if (f.severity) html += `<span class="finding-sev ${sevClass}">${escapeHtml(String(f.severity).toUpperCase())}</span>`;
                            html += '</div>';
                            if (f.note) html += `<p class="finding-note">${escapeHtml(f.note)}</p>`;
                            const metaBits = [];
                            if (f.location)  metaBits.push(`<span class="meta-kv"><span class="meta-k">Location</span><span class="meta-v">${escapeHtml(f.location)}</span></span>`);
                            if (f.file)      metaBits.push(`<span class="meta-kv"><span class="meta-k">File</span><span class="meta-v mono">${escapeHtml(f.file)}</span></span>`);
                            if (f.dataType)  metaBits.push(`<span class="meta-kv"><span class="meta-k">Type</span><span class="meta-v">${escapeHtml(f.dataType)}</span></span>`);
                            if (f.count != null && f.count > 1) metaBits.push(`<span class="meta-kv"><span class="meta-k">Count</span><span class="meta-v">${f.count}</span></span>`);
                            if (metaBits.length) html += `<div class="finding-meta">${metaBits.join('')}</div>`;
                            if (f.evidence) html += `<pre class="finding-evidence mono">${escapeHtml(String(f.evidence).slice(0, 800))}</pre>`;
                            if (Array.isArray(f.components) && f.components.length > 0) {
                                html += '<div class="finding-instances"><div class="finding-instances-head">Components (' + f.components.length + ')</div>';
                                for (const c of f.components.slice(0, 50)) {
                                    html += `<div class="finding-instance"><code class="mono">${escapeHtml(String(c))}</code></div>`;
                                }
                                if (f.components.length > 50) html += `<div class="finding-instance more">… ${f.components.length - 50} more</div>`;
                                html += '</div>';
                            }
                            if (Array.isArray(f.instances) && f.instances.length > 0) {
                                html += '<div class="finding-instances"><div class="finding-instances-head">Instances (' + f.instances.length + ')</div>';
                                for (const inst of f.instances.slice(0, 50)) {
                                    const head = (inst.file ? inst.file : '') + (inst.line && !inst.file ? '' : (inst.line ? ':' + inst.line : ''));
                                    html += '<div class="finding-instance">';
                                    if (head) html += `<div class="finding-instance-loc mono">${escapeHtml(head)}</div>`;
                                    if (inst.match) html += `<code class="mono">${escapeHtml(String(inst.match).slice(0, 300))}</code>`;
                                    else if (typeof inst.line === 'string' && inst.line.length > 0) html += `<code class="mono">${escapeHtml(String(inst.line).slice(0, 300))}</code>`;
                                    html += '</div>';
                                }
                                if (f.instances.length > 50) html += `<div class="finding-instance more">… ${f.instances.length - 50} more</div>`;
                                html += '</div>';
                            }
                            if (f.recommendation) html += `<p class="finding-rec">${escapeHtml(f.recommendation)}</p>`;
                            html += '</article>';
                        }
                        html += '</div>';
                    }
                    if (test.error) {
                        html += `<p style="margin-top:6px;color:var(--alert);font-size:12px">${escapeHtml(test.error)}</p>`;
                    }
                    html += '</div>';
                }
                results.innerHTML = html;
    ...
    function showClaimErrorModal(detail) {
    ...
    if (logoHome) logoHome.addEventListener('click', (e) => { e.preventDefault(); navigate('overview'); });
    document.querySelectorAll('[data-action]').forEach(el2 => {
        el2.addEventListener('click', (e) => {
            const action = el2.getAttribute('data-action');
            if (action === 'addUSB') { e.stopPropagation(); app.addUSBDevice(); }
            else if (action === 'showWebSocket') { e.stopPropagation(); app.showWebSocketModal(); }
            else if (action === 'closeTCP') app.closeTCPModal();
            else if (action === 'closeClaimError') closeClaimErrorModal();
        });
    });
    document.addEventListener('click', (e) => {
        const apkBtn = e.target.closest('.apk-download-btn');
        if (apkBtn) { app.downloadApk(apkBtn.dataset.pkg); return; }
        const viewBtn = e.target.closest('.storage-view-btn');
        if (viewBtn) { viewStorageFile(viewBtn.dataset.path); return; }
        const dlBtn = e.target.closest('.storage-dl-btn');
        if (dlBtn) { downloadStorageFile(dlBtn.dataset.pkg, dlBtn.dataset.path, dlBtn.dataset.name); return; }
    });
    ...
    navigate('overview');
    async function viewStorageFile(encodedPath) {
    ...
    async function downloadStorageFile(encodedPkg, encodedPath, encodedName) {
