...
MSAS.BackupScanner = (function() {
  ...
  function scan(opts) {
    ...
    // Note: Full manifest attribute detection (allowBackup, fullBackupContent) requires
    // parsing the AndroidManifest.xml which is available in result.manifest but not
    // directly exposed to scanners via the files array. This scanner relies on DEX string
    // detection of backup APIs. For complete coverage, the worker should pass
    // manifest backup attributes to this scanner.

    // Android backup assessment
    if (hasBackupAPI || hasAutoBackupRefs) {
      var backupDesc = '';
