...
MSAS.StateStore = (function () {
  ...
  function setState(key, value) {
    ...
    // Run middleware chain (allows mutation or veto)
    ...
    var newVal = middlewareResult !== undefined ? middlewareResult : value;
   ...
   * @param {*} newVal
   * @param {*} oldVal
   * @returns {*|undefined|false} transformed value, false to veto, or undefined
   */
  function runMiddleware(key, newVal, oldVal) {
    ...
    for (var i = 0; i < _middleware.length; i++) {
      ...
      if (result !== undefined) val = result;   // transform
    }
    return val;
  }
